...
const fail = process.argv[3] ?? ""; const ws = new WebSocket(`ws://127.0.0.1:${port}/ws${fail ? `?fail=${fail}` : ""}`);
...
ws.on("message", (d, isBinary) => {
  ...
  else if (m.type === "latency") { latencies.push(m); log.push(`+${Date.now()-t0} latency ${JSON.stringify({src:m.source, stt:m.sttFinalMs, tok:m.firstTokenMs, aud:m.firstAudioMs, tool:m.toolMs, total:m.totalMs, cancelled:m.cancelled, tts:m.ttsEngine})}`); }
  else if (m.type === "error") log.push(`+${Date.now()-t0} error ${m.message}`);
  else if (m.type === "ready") log.push(`+${Date.now()-t0} ready ${JSON.stringify(m.voice)} chaos=${JSON.stringify(m.chaos)}`);
});
ws.on("open", async () => {
